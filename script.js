...
function toggleMenu(forceClose = false) {
    ...
    if (menuOpen) {
        ...
        document.body.style.overflow = 'hidden'; // Locks body scroll on mobile
...
// Check if device supports hover (ignores touch devices for heavy mouse effects)
const isHoverSupported = window.matchMedia('(hover: hover) and (pointer: fine)').matches;

// 3. Custom Cursor Logic (Lerp + Magnetic)
if (isHoverSupported && window.innerWidth >= 1024) {
...
// 4. Spotlight Hover Effect & Hero Glow Tracking (Only active on pointer devices)
if (isHoverSupported) {
...
window.addEventListener('scroll', () => {
    if (!educationSection || !timelineProgress) return;
